...
class Attack:
    ...
    def ctc_output(self, audio, lengths, target):
        sess = self.sess
        ...
        for i in range(MAX):
            ...
            # Print out some debug information every 10 iterations.
            new, r_out, r_logits = sess.run((self.new_input, self.decoded, self.logits))
            
            # r_out, r_logits = sess.run((self.decoded, self.logits))
            ...
            for out, logits in lst:
                ...
                # if i == 0:
                #     joined = "".join(res)
                #     atten_y = split(joined)
                #     print("atten_y = split(res) ==", atten_y)
...
def attack_word(inp, lr, iterations, restore_path):
    
    wav_name, output_text = main_classify(inp, restore_path)
    ...
    with tf.Session() as sess:
        ...
        ctc_out = attack.ctc_output(audios, lengths, 
                                    [[toks.index(x) for x in phrase]]*len(audios))
        return wav_name, output_text, ctc_out
...
if __name__ == "__main__":
    ...
    wav_name, output_text = main_classify(args.input[0], args.restore_path)
    tf.reset_default_graph()
    with tf.Session() as sess:
        finetune = []
        audios = []
        ...
        ctc_out = attack.ctc_output(audios, lengths, 
                                    [[toks.index(x) for x in phrase]]*len(audios))
        print("type(ctc_out) = ", type(ctc_out))
        print("len(ctc_out) = ", len(ctc_out))
        text_file_name = args.input[0].split('.')
        text_file = 'ctcout_' + text_file_name[0] + '.txt'
        text_files = open(text_file, "w")
        text_files.write(ctc_out)
        text_files.close()
